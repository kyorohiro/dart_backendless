...
void main(List<String> args) {
  ArgParser parser = new ArgParser()..addOption('port', abbr: 'p', defaultsTo: '8080')..addOption('sport', abbr: 's', defaultsTo: '8443')..addOption('cert', abbr: 'c', defaultsTo: null)..addOption('work', abbr: 'w', defaultsTo: '/tmp')..addOption('ip', abbr: 'i', defaultsTo: '0.0.0.0');
...
void startWithSecure(String ip, int port, String path) {
  ...
  HttpServer.bindSecure(ip, port, context).then((HttpServer server) {
...
onRequest(HttpRequest request) {
  ...
  StringBuffer buffer = new StringBuffer("Origin, X-Requested-With, Content-Type, Accept");
  print("##A#${request.uri.path}#");
  var t = request.headers["access-control-request-headers"];
  for (String v in (t == null ? [] : t)) {
    buffer.write(",${v}");
  }
  print("${buffer.toString()}");
  request.response.headers.add("Access-Control-Allow-Headers", buffer.toString());
  print("##AA#${request.uri.path}#");
